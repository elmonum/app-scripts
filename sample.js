...
function LogTime(sheet_id, row_number) {
    ...
    //シートに記録
    result = LOG_SHEET.getRange(row_number, LOG_COL_POSITION, 1, time_list.length).setValues([time_list])
    //Logger.log(result)
}

function getKadoData(sheet_id) {
    var mySheet = SpreadsheetApp.openById(sheet_id).getSheetByName(KADO_SHEET_NAME);
    return mySheet.getRange(KADO_START_ROW, KADO_START_COL, 1, mySheet.getLastColumn() - 1).getValues()
}
